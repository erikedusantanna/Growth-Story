...
| Foco do mês e agenda | `calendar_system.gd`, `src/ui/screens/company_screen.gd` | No topo da aba **Empresa**: o **foco do mês** (vendas, entrega, gente ou caixa, escolhido uma vez por mês, com efeito em toda a operação) e uma **agenda curta** com os próximos 5 compromissos (fechamento do mês, prazos, missões, leads, aniversários), além do presente de **aniversário de contrato**, que melhora a relação. A tela de calendário dedicada foi cortada: era uma tela inteira para o que se usa de relance, e a grade de 12 meses ninguém reabria. As manchetes ficam na **banca**, na aba Agência |
...
| Recrutamento | `data/recruitment.json`, `recruitment_system.gd` | Esperar o lote mensal travava o crescimento, então há duas saídas pagas na aba Equipe: **buscas avulsas** (anúncio em plataforma, recrutadora freelancer, headhunter) que entregam currículos em poucos dias, e uma **recrutadora interna** contratada em tempo integral, que ocupa um lugar no escritório, soma candidatos ao lote de todo mês (com gente melhor que a média) e pode trazer alguém no meio do caminho. Dá para dispensá-la depois e liberar a vaga. O lote mensal espontâneo também subiu (2–3, +1 com reputação 25, +1 com 55) |
| Banco | `data/loans.json`, `bank_system.gd` | Botão 🏦 no World Map a partir da **Região 2**: quatro linhas de capital de giro (R$ 20 mil a 500 mil), liberadas por região e reputação. O dinheiro cai na conta na hora e a parcela fixa (tabela Price) sai no fechamento do mês. Sem caixa na hora do débito, a dívida ganha multa de 8% e a reputação cai. Dá para quitar o saldo de uma vez; no máximo 2 empréstimos abertos |
...
| RH | `hr_system.gd`, `data/hr_actions.json`, `hr_screen.gd` | Aba própria. Contrata-se uma analista (custo único + salário mensal; exige a **Sala no centro** — o primeiro escritório da Região 2 — e reputação 30; antes só abria na capital, tarde demais para quem já estava com o time grande) que ganha um anexo com divisória no escritório. Libera pizza, happy hour, feriado prolongado, energético com paçoca (buff), festa, bem-estar, retiro e as políticas pet friendly (cachorro e depois gato, permanentes no escritório, com moral diária) |
...
| UI | `src/ui/` | Tela inicial com a cidade da agência (fundo gerado em `tools/gen_art.py`, logo vindo da arte de marca), HUD, escritório, feed de humor, 6 abas (Equipe, Clientes, Projetos, Empresa, RH, Agência), modais |
| Marca (logo e ícone) | `assets/brand/`, `tools/gen_brand.py` | As duas artes originais ficam em `assets/brand/` (fora do APK) e o script deriva o que o jogo e o Android usam: logo da tela inicial (512 px), `icon.png` da janela (128), ícone legado do Android (192) e o ícone adaptativo (frente com a arte dentro da área segura, fundo com um degradê do céu dela, para nenhuma máscara de aparelho cortar o título). Para trocar a marca, substitua os dois arquivos em `assets/brand/` e rode `python3 tools/gen_brand.py` |
